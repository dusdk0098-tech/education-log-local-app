...
export function parseLaunchContext(argv) {
    ...
    for (let index = 0; index < argv.length; index += 2) {
        ...
        if (values.has(name))
            throw new LaunchContextError("LAUNCH_ARGUMENT_DUPLICATE");
    ...
    if (!APP_ID_PATTERN.test(appId))
        throw new LaunchContextError("LAUNCH_APP_ID_INVALID");
    ...
    if (!path.win32.isAbsolute(dataDirectory))
        throw new LaunchContextError("LAUNCH_DATA_DIR_INVALID");
